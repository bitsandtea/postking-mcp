...
export function registerDomainTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ brandId }) => {
      ...
      const data = await api.get<any>(`/api/domains?brandId=${id}`);
  ...
  server.tool(
    ...
    async ({ domain, primaryContentType, isPrimary, brandId }) => {
      const id = requireBrandId(brandId);
      const data = await api.post<any>(`/api/domains`, { brandId: id, domain, primaryContentType, isPrimary });
      return {
  ...
  server.tool(
    ...
    async ({ domainId }) => {
      const data = await api.post<any>(`/api/domains/${domainId}/verify`);
      return {
  ...
  server.tool(
    ...
    async ({ domainId }) => {
      await api.delete<any>(`/api/domains/${domainId}`);
      return {
  ...
  server.tool(
    ...
    async ({ publicationId, domainId, routingType, pathPrefix, brandId }) => {
      const id = requireBrandId(brandId);
      const data = await api.patch<any>(`/api/brands/${id}/publications/${publicationId}`, {
        domainId,
        routingType,
        pathPrefix,
      });
