...
export function registerBrandTools(server: McpServer) {
  ...
  server.tool(
    ...
    {},
    async () => {
      const data = await api.get<{ brands?: unknown[] }>("/api/brands");
  ...
  server.tool(
    ...
    async ({ brandId }) => {
      ...
      const data = await api.get<any>(`/api/brands/${id}`);
  ...
  server.tool(
    "create_brand",
    "Manually create a new brand without automatic website crawling.",
    {
  ...
  server.tool(
    "onboard_brand",
    "Crawl a website and automatically generate a brand profile with 10 content themes. Sets the new brand as active.",
    {
  ...
  server.tool(
    ...
    async ({ brandId }) => {
      const id = requireBrandId(brandId);
      const data = await api.get<unknown[]>(`/api/brands/${id}/themes`);
  ...
  server.tool(
    ...
    async ({ count, instructions, input, brandId }) => {
      ...
      const themes = await api.get<unknown[]>(`/api/brands/${id}/themes`);
