...
const PLATFORMS = ["x", "linkedin", "instagram", "threads", "facebook"];
function slimPost(p) {
    return {
        id: p.id,
        platform: p.platform,
...
export function registerPostTools(server) {
    ...
    server.tool("generate_post", [
        ...
        const { postId, pollUrl } = await api.post(`/api/brands/${id}/generate-content/async`, {
            medium: platform,
        ...
        const post = await pollUntilDone(pollUrl);
        let result = post;
    ...
    // ── Generate bulk posts ───────────────────────────────────────────────────
    server.tool("generate_bulk_posts", "Generate and schedule multiple posts across a date range in the background.", {
    ...
    server.tool("create_post", [
        ...
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/posts/manual`, {
            content,
            platforms,
    ...
    // ── List posts ────────────────────────────────────────────────────────────
    server.tool("list_posts", "List recent posts and drafts. Filter by status or platform. Use status='created' to find unscheduled drafts.", {
        ...
        const data = await api.get(`/api/brands/${id}/posts${qs}`);
    ...
    // ── View post ─────────────────────────────────────────────────────────────
    server.tool("get_post", "View the full content and status of a single post.", { postId: z.string().describe("Post ID") }, async ({ postId }) => {
        const data = await api.get(`/api/posts/${postId}`);
        const post = data?.post ?? data;
        return {
            content: [{ type: "text", text: JSON.stringify(slimPost(post), null, 2) }],
        };
    });
    // ── Approve post ──────────────────────────────────────────────────────────
    server.tool("approve_post", [
    ...
    // ── Reschedule post ───────────────────────────────────────────────────────
    server.tool("reschedule_post", "Move a scheduled post to a new time. Pass a future ISO 8601 UTC datetime.", {
    ...
    // ── Cancel / delete post ──────────────────────────────────────────────────
    server.tool("delete_post", "Cancel and delete a post regardless of its current status (draft, scheduled, or posted).", { postId: z.string().describe("Post ID to delete") }, async ({ postId }) => {
        await api.delete(`/api/posts/${postId}`);
        return {
            content: [{ type: "text", text: `Post ${postId} deleted.` }],
        };
    });
    // ── Calendar ──────────────────────────────────────────────────────────────
    server.tool("get_calendar", "View upcoming scheduled posts sorted by date.", {
        ...
        const data = await api.get(`/api/brands/${id}/posts?status=scheduled&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&limit=200`);
        const slim = unwrapPosts(data).map(slimPost);
        return {
            content: [{ type: "text", text: JSON.stringify(slim, null, 2) }],
        };
