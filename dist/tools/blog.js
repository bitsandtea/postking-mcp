...
export function registerBlogTools(server) {
    ...
    server.tool("list_blogs", "List all blog publications and their articles for the active brand. Publications are the hosting configs; articles are individual posts.", {
        ...
        const data = await api.get(`/api/brands/${id}/blogs`);
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ publications, articles }, null, 2) }],
        };
    });
    // ── Create publication ────────────────────────────────────────────────────
    server.tool("create_publication", "Create a new blog publication (the container that articles live under). Returns a publicationId needed for generate_blog_post.", {
        ...
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/blogs`, { title, description, layout });
        return {
            content: [{ type: "text", text: JSON.stringify({ id: data.id, title: data.title }, null, 2) }],
        };
    });
    // ── Generate blog post (AI) ───────────────────────────────────────────────
    server.tool("generate_blog_post", [
        ...
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/generate-blog`, {
    ...
    // ── Get blog article ──────────────────────────────────────────────────────
    server.tool("get_blog_article", "Fetch the full content of a blog article by ID.", {
        articleId: z.string().describe("Blog article ID"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ articleId, brandId }) => {
        const id = requireBrandId(brandId);
        const data = await api.get(`/api/brands/${id}/blogs/${articleId}`);
    ...
    // ── Update blog article ───────────────────────────────────────────────────
    server.tool("update_blog_article", "Edit a blog article — title, content, excerpt, SEO fields, status, author, or category. Set status='published' to make it live on your PostKing blog.", {
        articleId: z.string().describe("Blog article ID"),
        ...
        const id = requireBrandId(brandId);
        const data = await api.put(`/api/brands/${id}/blogs/${articleId}`, {
            postTitle: title,
            postText: content,
            postExcerpt: excerpt,
            status,
        ...
        return {
            content: [{ type: "text", text: JSON.stringify(slimArticle(data?.article ?? data), null, 2) }],
        };
    });
    // ── Delete blog article ───────────────────────────────────────────────────
    server.tool("delete_blog_article", "Permanently delete a blog article.", {
        articleId: z.string().describe("Blog article ID"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ articleId, brandId }) => {
        const id = requireBrandId(brandId);
    ...
    // ── List authors ──────────────────────────────────────────────────────────
    server.tool("list_blog_authors", "List all blog authors for the active brand. Author IDs can be passed to generate_blog_post or update_blog_article.", { brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)") }, async ({ brandId }) => {
        const id = requireBrandId(brandId);
        const data = await api.get(`/api/brands/${id}/authors`);
    ...
    // ── List categories ───────────────────────────────────────────────────────
    server.tool("list_blog_categories", "List all categories for a blog publication.", {
        publicationId: z.string().describe("Blog publication ID (from list_blogs)"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ publicationId, brandId }) => {
        const id = requireBrandId(brandId);
        const data = await api.get(`/api/brands/${id}/blogs/${publicationId}/categories`);
        const categories = (data?.categories ?? (Array.isArray(data) ? data : [])).map((c) => ({
            id: c.id,
            name: c.name,
            slug: c.slug,
    ...
    // ── Create category ───────────────────────────────────────────────────────
    server.tool("create_blog_category", "Create a new category in a blog publication.", {
        publicationId: z.string().describe("Blog publication ID"),
        ...
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/blogs/${publicationId}/categories`, { name, slug, description });
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    // ── List publishing connections ───────────────────────────────────────────
    server.tool("list_publishing_connections", "List external publishing connections for a blog publication (WordPress, Medium, Substack, etc.).", {
        publicationId: z.string().describe("Blog publication ID"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ publicationId, brandId }) => {
        const id = requireBrandId(brandId);
        const data = await api.get(`/api/brands/${id}/publications/${publicationId}/connections`);
        const connections = Array.isArray(data) ? data : (data?.connections ?? []);
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify(connections.map((c) => ({ id: c.id, platform: c.platform, label: c.label, autoPublish: c.autoPublish })), null, 2),
                },
            ],
        };
    });
    // ── Publish to external platforms ─────────────────────────────────────────
    server.tool("publish_blog_article", "Push a blog article to connected external platforms (WordPress, Medium, Substack, etc.). Get connection IDs from list_publishing_connections.", {
        articleId: z.string().describe("Blog article ID"),
        connectionIds: z.array(z.string()).describe("Connection IDs to publish to"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ articleId, connectionIds, brandId }) => {
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/blogs/${articleId}/publish`, { connectionIds });
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    // ── Import articles from external blog ────────────────────────────────────
    server.tool("import_blog_articles", "Import articles from an external blog, RSS feed, or Blogger URL into a PostKing publication as drafts.", {
        ...
        const id = requireBrandId(brandId);
        const data = await api.post(`/api/brands/${id}/publications/${publicationId}/import`, { sourceUrl, limit });
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    // ── SEO roadmap ───────────────────────────────────────────────────────────
    server.tool("get_seo_roadmap", "View the SEO content roadmap — suggested blog topics, keywords, and completion status.", {
        status: z.enum(["suggested", "in_progress", "completed", "ignored"]).optional().describe("Filter by status"),
        brandId: z.string().optional().describe("Brand ID (uses active brand if omitted)"),
    }, async ({ status, brandId }) => {
        const id = requireBrandId(brandId);
        const qs = status ? `?status=${status}` : "";
        const data = await api.get(`/api/brands/${id}/roadmap${qs}`);
        return {
            content: [
                {
                    type: "text",
